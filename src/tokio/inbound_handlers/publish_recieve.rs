...
pub struct PublishRecieveHandler<'p, P> where P: 'p + Send + Persistence {
    data_lock: FutMutex<LoopData<'p, P>>,
    state: Option<State>
}

impl<'p, P> PublishRecieveHandler<'p, P> where P: 'p + Send + Persistence {
...
impl<'p, P> Future for PublishRecieveHandler<'p, P> where P: 'p + Send + Persistence {
    type Item = ();
    type Error = Error;
